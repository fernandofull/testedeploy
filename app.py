...
def listar_produtos():
    if request.method == "POST":
        ...
        # Use o método or_ do sqlalchemy para combinar várias condições com "OU"
        search_filter = or_(
            Product.nome.ilike(f"%{termo}%"),
            Product.descricao.ilike(f"%{termo}%"),
            Product.ingredientes.ilike(f"%{termo}%")
        )

        # Consulta que utiliza o filtro de pesquisa
        resultado = db.session.query(Product).filter(search_filter).all()
